                ...
                <div class="flip-card">
                    <div class="flip-card-inner">
                        <div class="flip-card-front">
                            ...
                            <h2>{{ $item->tipo_servicio }} <span
                                    style="color: green; font-size: 12px;">Q.{{ $item->precio }}</span>
                            </h2>
                            <img src="/img/{{ $item->curriculum }}" alt="" style="margin-top: 20px">
                        </div>
                        <div class="flip-card-back">
                            <h5 style="text-align: center; font-weight: 900">{{ $item->telefono }}</h5>
                            <p style="line-height: 18px">{{ $item->correo }}</p>
                            <p style="text-align: center; text-decoration: underline; ">{{ $item->direccion }}</p>
                            <p style="text-align: center; text-decoration: underline; ">{{ $item->comentario }}</p>

                            <p style="margin-bottom: 5px; color: rgb(0, 255, 0)">{{ $item->estado }}</p>
                            @auth
                                <a href="{{ route('contratar') }}"><button class="bt">Contratar
                                    </button></a>
                            @endauth
                            @guest
